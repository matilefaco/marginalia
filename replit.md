...
- **Mock data** (`src/data/mockData.ts`) — 34 books (each with `bookColor`, optional `coverUrl` for real covers via Open Library), 71+ margins with threaded replies, 19 users with rich profiles, 6 editorial collections. Mock entries use `userId: "user_me"` — real authenticated users get empty states since their Supabase UUID won't match.
